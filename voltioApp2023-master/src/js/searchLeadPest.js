...
onAuthStateChanged(auth, async(user) => {
    if(user){
      ...
      async function getLeadOrProjectData(filter){
        ...
        if(!filter){
          let querySnapshoot
          const projectInfo = query(collection(db, 'leadData'), where('status', '==', status), where('project', '==', 'pestControl'));
          ...
          if(statusFilterData === ''){
            ...
            const projectInfo = query(collection(db, 'leadData'), where('status', '==', status), where('projectStatus', '==', statusFilterData), where('project', '==', 'pestControl'));
            querySnapshoot = await getDocs(projectInfo)
          }  
      ...
      searchLeadViewSection.forEach( (e) => { 
        ...
        e.addEventListener('click', async (e) => {
          ...
          const projectInfo = query(collection(db, 'leadData'), where('status', '==', 'lead'), where('project', '==', 'pestControl'));
